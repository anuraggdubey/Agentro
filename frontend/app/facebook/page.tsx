  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-1000 pb-12 w-full">
      ...
      <div className="flex items-center space-x-6">
        <div className="p-4 bg-[#1877F2]/10 glass-panel rounded-2xl shadow-xl border-[#1877F2]/20 transition-all duration-500">
          <Facebook className="text-[#1877F2]" size={32} />
        </div>
        <div>
          <h1 className="text-3xl font-black tracking-tighter text-foreground uppercase">FB Group <span className="text-gradient">Intelligence</span></h1>
          <p className="text-[10px] font-black text-muted/50 uppercase tracking-widest mt-1">Community signal analysis for high-engagement viral discussions.</p>
        </div>
      </div>
      ...
      <div className="glass-panel rounded-2xl p-8 relative overflow-hidden group shadow-2xl">
        <div className="flex flex-col md:flex-row items-center gap-6 relative z-10">
          <form onSubmit={handleSearch} className="relative flex-[2] group w-full">
            <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-muted group-focus-within:text-[#1877F2] transition-all duration-300" size={16} />
            <input 
              type="text" 
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder="Query FB Signals..." 
          ...
          <div className="relative flex-1 w-full group">
            ...
            <ChevronDown className="absolute right-5 top-1/2 -translate-y-1/2 text-muted pointer-events-none group-hover:text-[#1877F2] transition-colors" size={16} />
          </div>

          <button 
            onClick={handleGenerate}
            disabled={loading || !selectedTrend}
            className="px-10 py-4 bg-[#1877F2] text-white hover:bg-[#1877F2]/90 rounded-xl font-black text-[10px] tracking-[0.2em] shadow-2xl transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center whitespace-nowrap uppercase"
          >
            {loading ? <Loader2 className="animate-spin mr-3" size={16} /> : <TrendingUp size={16} className="mr-3" />}
            Analyze
          </button>
        </div>
            ...
            <button
              ...
              className={`px-5 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest border transition-all duration-300 ${
                selectedCategory === cat 
                  ? "bg-[#1877F2] border-[#1877F2] text-white shadow-lg shadow-[#1877F2]/20 scale-105" 
                  : "bg-card border-border text-muted hover:border-[#1877F2] hover:text-foreground"
              }`}
            >
              {cat}
            </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {[
          { title: "Trending Hubs", value: strategy?.reachEstimate || "8.4k+", icon: MessageCircle, label: "Active Nodes", color: "text-[#1877F2]" },
          { title: "Engagement IQ", value: strategy?.retentionSignal || "92%", icon: Target, label: "Response", color: "text-highlight" },
          { title: "Community Velocity", value: strategy?.velocityIndex || "High", icon: Users, label: "Network Growth", color: "text-primary" },
        ].map((card, i) => (
      ...
      <div className="glass-panel rounded-2xl overflow-hidden shadow-2xl relative group">
        <div className="p-1 px-10 py-6 border-b border-white/5 flex items-center justify-between relative z-10 bg-white/[0.02]">
          <div className="flex items-center space-x-4">
            ...
            <h2 className="text-xl font-black tracking-tighter text-white uppercase">Community Strategic <span className="text-gradient">Output</span></h2>
          </div>
          {strategy && (
            <button 
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
                  <div className="lg:col-span-2 space-y-10">
                    <div className="space-y-4">
                      <p className="text-[9px] font-black uppercase tracking-[0.4em] text-[#1877F2] border-l-2 border-[#1877F2] pl-6">Intel Vector</p>
                      <p className="text-3xl font-black leading-tight text-white uppercase tracking-tighter">{strategy.title}</p>
                    ...
                    <div className="space-y-6">
                      <p className="text-[9px] font-black uppercase tracking-[0.3em] text-muted/40 border-b border-white/5 pb-4">Community Engagement Map</p>
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                        {["Intercept", "Synthesize", "Deploy"].map((step, i) => (
                          <div key={i} className="glass-panel p-6 rounded-xl border-white/5 flex flex-col items-center justify-center space-y-2 text-center group/step hover:border-[#1877F2]/20 transition-all bg-white/2">
                            <span className="text-[10px] font-black text-[#1877F2]/40 group-hover/step:text-[#1877F2] transition-colors">{i + 1}</span>
                  ...
                  <div className="flex flex-col justify-between space-y-12 lg:border-l lg:border-white/5 lg:pl-16">
                    <div className="space-y-8">
                      <div className="flex flex-col items-center justify-center p-10 glass-panel rounded-2xl border-white/5 bg-white/2 text-center relative overflow-hidden shadow-inner">
                        <p className="text-[9px] font-black uppercase tracking-[0.3em] text-muted/40 mb-8">Virality Matrix</p>
                    ...
                    <div className="space-y-6">
                      <button className="w-full py-5 bg-[#1877F2] text-white rounded-xl font-black text-[10px] uppercase tracking-[0.3em] shadow-2xl hover:bg-[#1877F2]/90 active:scale-95 transition-all">
                        Publish to Groups
                      </button>
                      <p className="text-[8px] text-center text-muted/20 font-black uppercase tracking-[0.4em]">Community Intel Engine v2.0</p>
                    </div>
                  </div>
                </div>
              </motion.div>
                ...
                {loading ? (
                  <div className="space-y-6">
                    <div className="relative mb-10">
                       <div className="absolute inset-0 bg-[#1877F2]/20 blur-3xl rounded-full animate-pulse" />
                       <Loader2 className="animate-spin text-white relative z-10 mx-auto" size={48} />
                    </div>
                    <p className="text-white font-black text-xl tracking-tighter uppercase animate-pulse">Syncing Community Signals...</p>
                  </div>
                ) : (
                  <>
                    <>
                    <div className="p-10 glass-panel rounded-2xl bg-white/2 text-[#1877F2]/10 shadow-inner">
                      <Users size={60} />
                    </div>
                    <div className="max-w-md space-y-3">
                      <h3 className="text-xl font-black text-white tracking-widest uppercase">Intel Matrix Initialized</h3>
                      <p className="text-[10px] font-black text-muted/40 uppercase tracking-widest leading-relaxed">Select a priority community signal to initialize strategy engine.</p>
                    </div>
                  </>
                  </>
                )}
